...
- **Verification Engine**: Implemented `scripts/verify_skills.py` to run executable proofs for each skill.
- **Retrospective Engine**: Implemented `scripts/auto_retro.py` to automate the merging of failure insights into the skill registry.
- **Modular Validation Logic**: Introduced `scripts/skill_checkers.py` to separate metadata, content, and budget validation rules.
- **Standardized Docstrings**: Applied Google-style docstrings to all core scripts in the `scripts/` directory for enhanced maintainability.

### Changed

- **Governance Infrastructure**: Refactored `scripts/validate_memory.py` to utilize modular checkers and enforced mandatory semver bumps for Negative Knowledge updates.
- **Parser Robustness**: Hardened `scripts/yaml_parser.py` to handle complex nested objects and block scalars without external dependencies.
- **README Overhaul**: Completely rewritten `README.md` to reflect the new technical architecture and governance standards.

### Fixed

- Fixed recursive path resolution issues in `validate_memory.py` via `safe_relative_path`.
- Fixed frontmatter parsing edge cases where colons inside quotes or comments would disrupt metadata extraction.
